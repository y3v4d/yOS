...
const VIRTUAL_MODULE_ID = 'virtual:libs'
const RESOLVED_ID = '\0virtual:libs'
...
function findBuiltFile(rootDir: string, appName: string): string | null {
  const distDir = join(rootDir, 'dist', 'libs')
...
export function buildLibsPlugin(options: AppsPluginOptions = {}): Plugin {
  const {
    appsDir = 'apps',
    autoBuild = true,
  ...
  async function loadAllApps(isDev: boolean) {
    ...
    for (const appName of appNames) {
      ...
      if (autoBuild && configFile) {
        ...
        if (!builtFile || isDev) {
          console.log(`[vite-plugin-libs] Building ${appName}...`)
      ...
      if (isDev) {
        const builtFile = findBuiltFile(rootDir, appName)
        if (!builtFile) {
          console.warn(`[vite-plugin-libs] No built file found for app "${appName}"`)
          continue
        }
        const source = readFileSync(builtFile, 'utf-8')
        appCache.set(key, source)
        console.log(`[vite-plugin-libs] Loaded lib "${key}" (${(source.length / 1024).toFixed(1)}kb)`)
      } else {
        // prod: store the URL — file will be copied to dist/apps/ in closeBundle
        const url = `${prodBasePath}/${appName}.js`
        appCache.set(key, url)
        console.log(`[vite-plugin-libs] Registered lib "${key}" -> ${url}`)
      }
    }
  }
  ...
  function generateVirtualModule(isDev: boolean): string {
    ...
    if (isDev) {
      return [
        `export const libs = {\n${entries}\n};`,
        `export const libUrls = undefined;`,
        `export default libs;`,
      ].join('\n')
    } else {
      return [
        `export const libs = undefined;`,
        `export const libUrls = {\n${entries}\n};`,
        `export default libUrls;`,
      ].join('\n')
    }
  }

  return {
    name: 'vite-plugin-libs',
    enforce: 'pre',
      ...
      if (isDev && watchMode) {
        for (const appName of getAppDirs(resolvedAppsDir)) {
          ...
          watch(watchTarget, { recursive: true }, async (event, filename) => {
            ...
            console.log(`[vite-plugin-libs] "${appName}" changed, rebuilding...`)
            try {
              await buildApp(appDir, configFile)
              const builtFile = findBuiltFile(rootDir, appName)
              if (builtFile) {
              ...
              console.error(`[vite-plugin-libs] Build failed for "${appName}":`, e)
            }
          })
        }
      }
    ...
    async closeBundle() {
        ...
        for (const appName of getAppDirs(resolvedAppsDir)) {
            ...
            const configFile = configCandidates.find(existsSync)
            if (!configFile) {
                console.warn(`[vite-plugin-libs] No vite config found for "${appName}"`)
                continue
            }
            console.log(`[vite-plugin-libs] Building ${appName} into dist/libs/...`)
            await buildApp(appDir, configFile)
        }
    },
