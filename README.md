...
- **Virtual filesystem** — an inode and block based VFS backed by IndexedDB for persistence across page loads. Supports file descriptors, `open`,  `mkdir`, `rmdir`, `unlink`, `read`, `write`, `lseek`, `fseek`, `opendir`, `readdir` and standard path resolution. It also supports special file descriptors for inotify-like file watching.
...
![yOS File Manager](./docs/yos-file-manager.png)
